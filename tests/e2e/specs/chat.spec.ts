 ...
 *   AICO_BOT_TEST_API_KEY - API key for testing
 *   AICO_BOT_TEST_API_URL - API URL (optional)
 *   AICO_BOT_TEST_MODEL   - Model to use (optional)
 */
...
test.beforeEach(async ({}, testInfo) => {
  if (!hasApiKey()) {
    testInfo.skip(true, 'Skipping chat tests: AICO_BOT_TEST_API_KEY not set')
  }
})

/**
 * Helper to navigate from Home Page to Chat Interface
 * The app shows Home Page first, need to click "进入 AICO-Bot" to get to chat
 */
async function navigateToChat(window: any) {
  await window.waitForSelector('#root', { timeout: 10000 })
  await window.waitForLoadState('networkidle')

  // Look for "进入 AICO-Bot" text button (the main AICO-Bot temp space card)
  // Try multiple approaches
  let enterAicoBot = await window.waitForSelector(
    'text=/进入 AICO-Bot/',
    { timeout: 5000 }
  ).catch(() => null)

  if (!enterAicoBot) {
    // Fallback: look for any element with "AICO-Bot" text that's clickable
    enterAicoBot = await window.waitForSelector(
      ':text("AICO-Bot"):visible',
      { timeout: 5000 }
    ).catch(() => null)
  }

  if (enterAicoBot) {
    await enterAicoBot.click()
  }
...
test.describe('Chat Interface', () => {
  test('chat input is visible and functional', async ({ window }) => {
    ...
    await chatInput.fill('Hello, AICO-Bot!')
    const value = await chatInput.inputValue()
    expect(value).toBe('Hello, AICO-Bot!')
  })
...
test.describe('Real Chat Flow', () => {
  ...
  test('can send message and receive response', async ({ window }) => {
    ...
    // Wait for AI to finish working (wait for "AICO-Bot 工作中" to disappear)
    await window.waitForSelector(
      'text="AICO-Bot 工作中"',
  ...
  test('displays thinking indicator during response', async ({ window }) => {
    ...
    // Look for working indicator ("AICO-Bot 工作中")
    ...
    await window.waitForSelector('text="AICO-Bot 工作中"', { state: 'hidden', timeout: 45000 }).catch(() => {})
...
test.describe('Switch Provider and Chat', () => {
  ...
  test('switch to tencent provider, select GLM-5.0, and chat', async ({ window }) => {
    ...
    await window.waitForSelector('text="AICO-Bot 工作中"', { state: 'hidden', timeout: 60000 }).catch(() => {})

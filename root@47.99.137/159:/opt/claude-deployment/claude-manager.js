...
function isGitRepo(dir) {
    try {
        ...
        while (currentDir !== path_1.default.dirname(currentDir)) {
            if (fs.existsSync(path_1.default.join(currentDir, '.git'))) {
                return true;
            }
            currentDir = path_1.default.dirname(currentDir);
        }
        // Check root level
        return fs.existsSync(path_1.default.join(currentDir, '.git'));
    }
    catch {
        return false;
    }
 ...
 */
function buildSystemPrompt(workDir, modelInfo) {
    const today = new Date().toISOString().split('T')[0];
    const isGit = isGitRepo(workDir);
    // Try to load synced system prompt from file
    const systemPromptPath = path_1.default.join('/opt/claude-deployment', 'config', 'system-prompt.txt');
    try {
 ...
 */
function isSessionTransportReady(session) {
    try {
 ...
 */
function registerProcessExitListener(session, conversationId, onExit) {
    try {
 ...
 */
function needsSessionRebuild(existing, newConfig) {
    return (existing.config.model !== newConfig.model ||
...
class ClaudeManager {
    ...
    sessions = new Map(); // conversationId -> V2SessionInfo
    activeSessions = new Map(); // In-flight requests
    // Configuration
    apiKey;
    baseUrl;
    ...
    constructor(apiKey, baseUrl, pathToClaudeCodeExecutable, workDir, model) {
        this.apiKey = apiKey;
     ...
     */
    updateConfig(apiKey, baseUrl, workDir, model) {
        this.apiKey = apiKey;
        this.baseUrl = baseUrl;
        this.workDir = workDir || process.cwd();
        this.model = model;
     ...
     */
    async getOrCreateSession(conversationId, workDir) {
        const effectiveWorkDir = workDir || this.workDir || process.cwd();
        ...
        const session = (0, claude_agent_sdk_1.unstable_v2_createSession)(options);
    ...
    getSession(sessionId) {
        ...
        if (!session) {
            ...
            session = (0, claude_agent_sdk_1.unstable_v2_createSession)(options);
     ...
     */
    registerActiveSession(conversationId, abortController) {
    ...
    async resumeSession(sessionId) {
        if (!this.sessions.has(sessionId)) {
            const options = this.buildSdkOptions();
            const session = await (0, claude_agent_sdk_1.unstable_v2_resumeSession)(sessionId, options);
            registerProcessExitListener(session, sessionId, (id) => {
                this.cleanupSession(id, 'process exited');
            });
            this.sessions.set(sessionId, {
     ...
     */
    async *streamChat(sessionId, messages, options = {}, onToolCall, onTerminalOutput, onThought, onThoughtDelta, onMcpStatus, onCompact) {
        ...
        try {
            ...
            for await (const event of session.stream()) {
                ...
                // System events - MCP status and compact boundary
                if (evt.type === 'system') {
                    const subtype = evt.subtype;
                    // Handle compact_boundary - context compression notification
                    if (subtype === 'compact_boundary') {
                        const compactMetadata = evt.compact_metadata;
                        if (compactMetadata) {
                            onCompact?.({
                    ...
                    const mcpServers = evt.mcp_servers;
                ...
                if (evt.type === 'assistant') {
                    ...
                    console.log(`[ClaudeManager] Assistant event - message.content types:`, message?.content?.map((b) => b.type));
     ...
     */
    async chat(sessionId, messages, options = {}) {
        const session = await this.getOrCreateSession(sessionId, options.workDir);
        const abortController = new AbortController();
        this.registerActiveSession(sessionId, abortController);
        try {

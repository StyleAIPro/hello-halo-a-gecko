...
function renderFileChanges(fileChanges: Message['metadata']['fileChanges']): string {
  ...
  lines.push(`> Total: ${fileChanges.totalAdded} added, ${fileChanges.totalRemoved} removed across ${fileChanges.totalFiles} file(s)`)
  return lines.join('\n')
}
...
export function conversationToMarkdown(conversation: Conversation): string {
  ...
  lines.push('')
  lines.push(`> Created: ${formatTimestamp(conversation.createdAt)} | Updated: ${formatTimestamp(conversation.updatedAt)}`)

...
export function useTextSelection(options: UseTextSelectionOptions): UseTextSelectionReturn {
  ...
  // 计算菜单位置
  const calculateMenuPosition = useCallback((view: EditorView, from: number): { x: number; y: number } => {
  ...
  const setSelectionManual = useCallback((newSelection: TextSelection | null) => {
    setSelection(newSelection)
    if (newSelection) {
      onSelectionChange?.(newSelection)
    } else {

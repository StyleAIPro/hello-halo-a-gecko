...
function StreamingBubble({
  ...
  useEffect(() => {
    if (textBlockVersion !== prevTextBlockVersionRef.current) {
      console.log(
        `[StreamingBubble] 🆕 New text block detected: version ${prevTextBlockVersionRef.current} → ${textBlockVersion}`,
      );
  ...
  const displayContent =
    activeSnapshotLen > 0 && content.length >= activeSnapshotLen
  ...
  return (
    <div className="rounded-2xl px-4 py-3 message-assistant message-working w-full overflow-hidden">
      ...
      <div className="flex items-center gap-1.5 mb-2 pb-2 border-b border-border/30 working-indicator-fade">
        <span className="text-xs text-muted-foreground/70">{t('AICO-Bot is working')}</span>
      </div>
      ...
      >
        ...
        >
          ...
          <div ref={historyRef}>
            {segments.map((seg, i) => (
              <div key={i}>
                ...
                {i < segments.length - 1 && <div className="border-t border-border/40 my-3" />}
              </div>
            ))}
          </div>
          ...
          <div ref={currentRef} className="break-words leading-relaxed">
            ...
            {!isStreaming && <span className="waiting-dots ml-1 text-muted-foreground/60" />}
          </div>
        </div>
      </div>
    </div>
...
export const MessageList = forwardRef<MessageListHandle, MessageListProps>(function MessageList(
  ...
  }, [isGenerating, error, errorType, compactInfo, t, contentWidthClass]);

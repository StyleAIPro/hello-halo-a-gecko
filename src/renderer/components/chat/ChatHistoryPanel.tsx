...
import { X, EllipsisVertical, Pin, Pencil, Trash2, Download } from 'lucide-react';
...
export function ChatHistoryPanel() {
  ...
  const spaceName = currentSpace?.isTemp ? t('AICO-Bot Space') : (currentSpace?.name ?? '');
  ...
  const renderHistoryItem = useCallback(
    (_index: number, conv: ConversationMeta) => (
      <div
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 relative">
          ...
          {editingId !== conv.id && (
            <>
              <div
                className={`absolute right-0 top-[1px] ${isMobile ? 'block' : 'hidden group-hover:block'}`}
              >
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        if (spaceId)
                          useChatStore
                            .getState()
                            .toggleStarConversation(spaceId, conv.id, !conv.starred);
                        setMenuOpenId(null);
                      }}
      ...
      {conversations.length === 0 ? (
        <div className="px-4 py-8 text-center">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">
            {t('Start a new conversation to create history')}
          </p>
        </div>
      ) : (
        <div style={{ height }}>
          <Virtuoso data={conversations} overscan={200} itemContent={renderHistoryItem} />
        </div>
      )}
    </>
  );

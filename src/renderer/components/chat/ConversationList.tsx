...
import { ChevronLeft, ChevronRight, EllipsisVertical, Pin, Pencil, Trash2, Download, Square, Wrench } from 'lucide-react'
            ...
            {statusDetail && status !== 'idle' && (
              <div className="flex items-center gap-1.5 mt-0.5 ml-5">
                <span className={`text-xs ${
                  ...
                  status === 'completed-unseen' ? 'text-green-500' :
...
export const ConversationList = memo(function ConversationList({
  ...
  // Menu toggle handler
  const handleMenuToggle = useCallback((conversationId: string) => (e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  const renderConversationItem = useCallback((conversation: ConversationMeta) => (
    <ConversationItem
      ...
      onSelect={() => editingId !== conversation.id && useChatStore.getState().selectConversation(conversation.id)}
      onMenuToggle={handleMenuToggle(conversation.id)}
      onMenuClose={() => { setMenuOpenId(null); setMenuPosition(null) }}
      onToggleWorkers={() => handleToggleWorkers(conversation.id)}
    />
  ), [currentConversationId, statusDetails, editingId, editingTitle, menuOpenId, isHyperSpace, workerConversations, workersExpandedId, handleEditKeyDown, handleSaveEdit, handleMenuToggle, handleToggleWorkers])

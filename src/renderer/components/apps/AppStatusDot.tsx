...
export function AppStatusDot({
  ...
  if (effective === 'idle' || status === 'active') {
    return <span className={`inline-block ${sz} rounded-full bg-green-500/50 ${className}`} />;
  }
  if (status === 'waiting_user' || effective === 'waiting_user') {
    return <span className={`inline-block ${sz} rounded-full bg-orange-400 ${className}`} />;
  }
  if (status === 'error' || effective === 'error') {
    return <span className={`inline-block ${sz} rounded-full bg-red-500 ${className}`} />;
  }
  if (status === 'paused' || effective === 'paused') {
    return (
      <span
        className={`inline-block ${sz} rounded-full border border-muted-foreground/40 ${className}`}
      />
    );
  }
  if (status === 'needs_login') {
    return <span className={`inline-block ${sz} rounded-full bg-yellow-400 ${className}`} />;
  }
  ...
  return <span className={`inline-block ${sz} rounded-sm bg-muted-foreground/40 ${className}`} />;
}

...
import { X, Minimize2, Maximize2, Trash2, Terminal as TerminalIcon, Send, Sparkles, ChevronDown, ChevronUp } from 'lucide-react'
...
export function TerminalPanel({ spaceId, conversationId, onGenerateSkill, isVisible: parentIsVisible }: TerminalPanelProps) {
  ...
  useEffect(() => {
    ...
    term.writeln('\x1b[1;36m' + '║' + '  🤖 Shared Terminal - Human-Agent Collaboration  '.padEnd(55) + '║' + '\x1b[0m')
    term.writeln('\x1b[1;36m' + '╚═══════════════════════════════════════════════════════════╝' + '\x1b[0m')
    term.writeln('')
  ...
  useEffect(() => {
    ...
    commands.forEach(cmd => {
      const sourceColor = cmd.source === 'agent' ? '\x1b[1;34m[AGENT]\x1b[0m' : '\x1b[1;32m[USER]\x1b[0m'
    ...
    <div className={getPanelClass()}>
      ...
      <div className="terminal-header">
        <div className="terminal-title">
          ...
          {isConnected && (
            <span className="terminal-status connected">{t('Connected')}</span>
          )}
        </div>
      ...
      {isExpanded && (
        <div className="terminal-help">
          <span>Ctrl+` {t('toggle')} • / {t('focus')} • Esc {t('blur')}</span>
        </div>
      )}
    </div>
  )

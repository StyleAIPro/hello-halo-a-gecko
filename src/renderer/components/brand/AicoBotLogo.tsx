/**
 * AicoBotLogo - Brand animated logo component
 * Used across the app for loading states and branding
 *
 * Usage:
 *   <AicoBotLogo size="sm" />      // 28px - for inline/small areas
 *   <AicoBotLogo size="md" />      // 48px - for medium contexts
 *   <AicoBotLogo size="lg" />      // 96px - for large displays (like splash)
 *   <AicoBotLogo size={64} />      // custom size in pixels
 */
...
export function AicoBotLogo({ size = 'md', className = '' }: AicoBotLogoProps) {
  ...
  return (
    ...
    >
      ...
      {/* Main ring */}
      <div
        className={`relative rounded-full ${styles.border} border-primary/60 flex items-center justify-center aico-bot-glow`}
        style={{ width: pixelSize, height: pixelSize }}
      >
        {/* Inner glow */}

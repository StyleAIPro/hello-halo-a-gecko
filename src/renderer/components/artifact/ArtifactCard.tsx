  ...
  return (
    <div
      ...
      className={`
        ...
        ${isHovered ? 'bg-secondary shadow-sm' : 'bg-secondary/50 hover:bg-secondary/80'}
      `}
      title={
        canViewInCanvas
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium truncate text-foreground/90">{artifact.name}</p>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            {!isFolder && artifact.size !== undefined && <span>{formatSize(artifact.size)}</span>}
            {isFolder && <span>{t('Folder')}</span>}
          </div>
        </div>

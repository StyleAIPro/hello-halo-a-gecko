...
export function ArtifactTree({ spaceId }: ArtifactTreeProps) {
  ...
  const loadTree = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        '[ArtifactTree] loadTree IPC response: success=%s, nodeCount=%d',
        response.success,
  ...
  const loadChildren = useCallback(
    async (dirPath: string): Promise<void> => {
      ...
      try {
        ...
        console.log(
          '[ArtifactTree] loadChildren IPC response: success=%s, childCount=%d, path=%s',
          response.success,
          response.data?.length ?? 0,
          dirPath,
        );
        if (response.success && response.data) {
          ...
          if (parent) {
            ...
            setRevision((r) => r + 1);
            console.log(
              '[ArtifactTree] loadChildren OK: %d children attached to "%s", hasChildren=%s',
              children.length,
              parent.name,
            ...
            console.warn(
              '[ArtifactTree] loadChildren: parent NOT in nodeIndex — path=%s, indexSize=%d',
              dirPath,
              nodeIndex.current.size,
            );
          }
        } else {
          console.warn(
            '[ArtifactTree] loadChildren: response not successful or empty — path=%s',
            dirPath,
          );
        }
      } catch (error) {
      ...
      for (const { dirPath, children } of event.updatedDirs) {
        ...
        if (parent) {
          ...
          if (isRoot || treeDataRef.current.length === 0) {
            treeDataRef.current = mergeChildren(
              incomingChildren,
              treeDataRef.current,
              nodeIndex.current,
            );
...
function TreeNodeComponent({ node, style, dragHandle }: NodeRendererProps<ArtifactTreeNode>) {
  ...
  const handleToggle = useCallback(async () => {
    if (!isFolder) return;
    console.log(
      '[ArtifactTree] handleToggle: name="%s", isOpen=%s, isLeaf=%s, childrenLoaded=%s, dataChildren=%s',
      data.name,
      node.isOpen,
      node.isLeaf,
      data.childrenLoaded,
    ...
    node.toggle();
    console.log(
      '[ArtifactTree] handleToggle DONE: name="%s", toggled to open=%s',
      data.name,
      !node.isOpen,
    );
  ...
  return (
    ...
    >
      ...
      {/* File/folder icon */}
      <span
        className={`w-4 h-4 flex items-center justify-center flex-shrink-0 mr-1.5 ${dimmed ? 'opacity-50' : ''}`}
      >
        <FileIcon

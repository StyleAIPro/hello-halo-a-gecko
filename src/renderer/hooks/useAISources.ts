...
export function useAISources({ config, setConfig, updateConfig }: UseAISourcesOptions): UseAISourcesResult {
  ...
  const switchSource = useCallback(
    async (sourceId: string) => {
      ...
      if (result.success && result.data) {
        updateConfig({ aiSources: result.data as AISourcesConfig });
      }
    },
    [updateConfig],
  );

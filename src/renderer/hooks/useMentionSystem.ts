...
export function useMentionSystem({
  ...
  const handleMentionKeyDown = useCallback((e: KeyboardEvent<HTMLTextAreaElement>): boolean => {
      ...
      switch (e.key) {
        case 'ArrowDown':
          ...
          setSelectedMentionIndex(prev =>
            prev < displayMembers.length - 1 ? prev + 1 : 0
          )
          return true
        ...
        case 'ArrowUp':
          e.preventDefault()
          setSelectedMentionIndex(prev =>
            prev > 0 ? prev - 1 : displayMembers.length - 1
          )
          return true

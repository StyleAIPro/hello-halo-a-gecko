...
function persistLayout(update: Partial<LayoutConfig>) {
  ...
  api
    .setConfig({ layout: update })
    .catch((err) => console.error('[SpacePage] Failed to persist layout:', err));
}
...
export function SpacePage() {
  ...
  const [showConversationList, setShowConversationList] = useState(sidebarOpenConfig ?? false);
  ...
  useEffect(() => {
    ...
    const initSpace = async () => {
      ...
      const { defaultSpace, spaces } = useSpaceStore.getState();
      const allSpaceIds = [
    ...
    <div className="h-full w-full flex flex-col">
      ...
      */}
          ...
          right={
            <>
              ...
              <button
                ...
                className={`flex items-center gap-1 px-2 py-1.5 hover:bg-secondary rounded-lg transition-colors text-xs`}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {!isMobile && currentSpace?.spaceType === 'hyper' && !isCanvasMaximized && <AgentPanel />}
              ...
              >
                ...
                {!showConversationList && (
                  <div className="absolute top-2 left-0 z-10">
                    <SidebarToggle isOpen={false} onToggle={handleToggleConversationList} />
                  </div>
                )}

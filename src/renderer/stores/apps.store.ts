  ...
  reinstallApp: async (appId) => {
    try {
      ...
      if (res.success) {
        ...
        set((state) => ({
          apps: state.apps.map((a) =>
            a.id === appId ? { ...a, status: 'active' as AppStatus, uninstalledAt: undefined } : a,
          ),
        }));
        return true;
      }
    ...
    try {
      ...
      if (res.success) {
        ...
        set((state) => {
          ...
          const updated = entries.map((e) => (e.id === escalationId ? { ...e, userResponse } : e));
          return {
  ...
  handleStatusChanged: (appId, state) => {
    set((s) => {
      ...
      return {
        ...
        apps: s.apps.map((a) => (a.id === appId ? { ...a, status: appStatus } : a)),
      };
    });
  },

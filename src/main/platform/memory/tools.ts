...
export function createMemoryMcpServer(
  ...
  const memory_read = tool(
      ...
      `If a 'path' is specified, reads a specific file from the memory archive directory (mode is ignored for archive reads).`,
    {
      ...
      mode: z
        .enum(['full', 'headers', 'section', 'tail'])
        .default('full')
        .describe(
      ...
      limit: z
        .number()
        .optional()
        .describe('Number of lines to return when mode="tail". Defaults to 50.'),
      path: z
        .string()
        .optional()
        .describe(
  ...
  const memory_write = tool(
    ...
    async (args) => {
      try {
        ...
        const actualMode = args.mode as 'append' | 'replace';
  ...
  const memory_list = tool(
    ...
    {
      scope: readScopeEnum.describe(`Memory scope to list archive files from.`),
    },
    async (args) => {
      try {

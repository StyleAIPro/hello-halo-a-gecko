...
import type { SchedulerJob, JobDueHandler, RunOutcome, JobStatus } from './types';
...
export class SchedulerTimer {
  ...
  start(): void {
    ...
    if (clearedCount > 0) {
      console.log(
        `[Scheduler] Cleared ${clearedCount} stale running marker(s) from previous session`,
      );
    }

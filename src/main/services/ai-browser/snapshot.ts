...
interface CDPAXNode {
  ...
  name?: {
    type: string;
    value: string;
    sources?: Array<{ type: string; value?: { type: string; value: string } }>;
  };
...
export async function createAccessibilitySnapshot(
  ...
  try {
    ...
    const rootCDPNode = response.nodes.find((n) => !n.ignored && !n.parentId) || response.nodes[0];
 ...
 */
export async function focusElement(webContents: WebContents, backendNodeId: number): Promise<void> {
  try {
    // Ensure debugger is attached
    try {
      webContents.debugger.attach('1.3');

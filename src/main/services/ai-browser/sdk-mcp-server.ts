...
async function withRetry<T>(
  ...
  try {
    ...
    const isElementNotFound =
      errorMsg.includes('Element not found') || errorMsg.includes('not found');
    ...
    console.log(
      `[sdk-mcp-server] ${label} failed with "${errorMsg}", retrying with fresh snapshot...`,
    );
...
export function buildAllTools(ctx: BrowserContext) {
  ...
  const browser_new_page = tool(
    ...
    async (args) => {
      ...
      try {
        ...
        return textResult(
          `Created new page: ${finalState?.title || 'Untitled'} - ${finalState?.url || args.url}`,
        );
      } catch (error) {
  ...
  const browser_navigate = tool(
    ...
    async (args) => {
      ...
      try {
        ...
        return textResult(
          `Unable to navigate in the selected page: ${(error as Error).message}.`,
          true,
        );
      }
    },
  ...
  const browser_handle_dialog = tool(
    ...
    async (args) => {
      ...
      try {
        ...
        return textResult(
          `Successfully ${args.action === 'accept' ? 'accepted' : 'dismissed'} the dialog`,
        );
      } catch (error) {
  ...
  const browser_hover = tool(
    ...
    async (args) => {
      ...
      try {
        ...
        await withTimeout(
          withRetry(async () => ctx.hoverElement(args.uid), 'Hover element', ctx, args.uid),
          TOOL_TIMEOUT,
          'browser_hover',
        );
  ...
  const browser_press_key = tool(
    ...
    async (args) => {
      ...
      try {
        await withTimeout(ctx.pressKey(args.key), TOOL_TIMEOUT, 'browser_press_key');
  ...
  const browser_screenshot = tool(
    ...
    'Take a screenshot of the page or element.',
    {
      ...
      quality: z
        .number()
        .optional()
        .describe('Compression quality for JPEG and WebP formats (0-100). Ignored for PNG format.'),
      uid: z
        .string()
        .optional()
        .describe(
...
`),
    ...
    async (args) => {
      try {
        ...
        for (const msg of pageMessages) {
          ...
          if (msg.url) {
            lines.push(
              `    at ${msg.url}${msg.lineNumber !== undefined ? `:${msg.lineNumber}` : ''}`,
            );
          }
          lines.push('');
        }
  ...
  const browser_emulate = tool(
    ...
    {
      networkConditions: z
        .enum(['No emulation', 'Offline', 'Slow 3G', 'Fast 3G', 'Regular 4G', 'DSL', 'WiFi'])
        .optional()
        .describe(
      ...
      try {
        ...
        if (args.geolocation !== undefined) {
          if (args.geolocation === null) {
            ...
            results.push(
              `Geolocation: ${args.geolocation.latitude}, ${args.geolocation.longitude}`,
            );
          }
        }
  ...
  const browser_perf_start = tool(
    ...
    async (args) => {
      ...
      try {
        ...
        return textResult(
          'The performance trace is being recorded. Use browser_perf_stop to stop it.',
        );
      } catch (error) {

...
export class ConversationAnalyzer {
  ...
  private async analyzeToolPattern(
    ...
    const toolStats: Record<
      string,
      { count: number; successCount: number; errorCount: number; totalInputSize: number }
    > = {};
  ...
  private extractCommonInputs(
    ...
    for (const [name, inputs] of Object.entries(inputsByName)) {
      if (inputs.length >= 2) {
        ...
        const commonKeys = keys.filter((key) => inputs.every((input) => key in input));
  ...
  private assessReusability(
    ...
    // 3. 成功率 (0-0.2)
    const totalSuccess = Object.values(toolPattern.toolStats).reduce(
      (sum, s) => sum + s.successCount,
      0,
    );
    const totalCount = Object.values(toolPattern.toolStats).reduce((sum, s) => sum + s.count, 0);

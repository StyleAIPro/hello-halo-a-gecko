  ...
  if (response.status === 403) {
    ...
    if (rateLimitRemaining === '0') {
      const resetDate = rateLimitReset
        ...
        : 'unknown'
      throw new Error(`GitHub API rate limit exceeded. Resets at ${resetDate}. Authenticate with gh CLI to increase limits.`)
    }
...
export async function listRepoDirectories(
  ...
  try {
    ...
    console.log(`[GitHubSkillSource] listRepoDirectories response type: ${typeof data}, isArray: ${Array.isArray(data)}`)
    if (!Array.isArray(data)) {
      console.warn(`[GitHubSkillSource] listRepoDirectories unexpected response:`, JSON.stringify(data).slice(0, 200))
      return []
    }
    const dirs = data
...
export async function fetchSkillFileContent(
  ...
  if (token) {
    try {
      const data = await githubApiFetch(`/repos/${repo}/contents/${encodeURIComponent(path).replace(/%2F/g, '/')}`, { token })
...
export async function validateRepo(
  ...
  try {
    ...
    if (!repoData) {
      return { valid: false, hasSkillsDir: false, skillCount: 0, error: 'Repository not found or is private' }
    }
...
export async function pushSkillAsPR(
  ...
  try {
    ...
    const { stdout: userJson } = await execAsync(
      `"${ghBin}" api user --jq ".login"`,
      { timeout: 15_000 }
    )
    ...
    try {
      const { stdout: isFork } = await execAsync(
        `"${ghBin}" api /repos/${repo} --jq ".fork"`,
        { timeout: 10_000 }
      )
      if (isFork.trim() === 'true') {
    ...
    if (targetRepo === repo && prTargetRepo === repo) {
      try {
        await execAsync(
          `"${ghBin}" api /repos/${repo}/collaborators/${username} --jq ".login"`,
          { timeout: 10_000 }
        )
        ...
        try {
          await execAsync(
            `"${ghBin}" repo fork ${repo} --clone=false`,
            { timeout: 30_000 }
          )
        } catch (forkError: any) {
    ...
    console.log(`[GitHubSkillSource] Committing ${files.length} file(s) to ${targetRepo}:${branchName}...`)
 ...
 */
export async function readLocalSkillContent(skillId: string): Promise<{ content: string; fileName: string } | null> {
 ...
 */
export async function readLocalSkillFiles(skillId: string): Promise<Array<{ relativePath: string; content: string }>> {
  const skillsDir = getAgentsSkillsDir()
  const skillDir = join(skillsDir, skillId)

...
export function calculateSimilarity(
  ...
  // 3. 语义相似度 (权重: 0.2)
  const semanticScore = compareSemantics(analysis.userIntent.primaryGoal, skill.spec.description);
  ...
  // 4. 关键词匹配 (权重: 0.2)
  const keywordScore = compareKeywords(analysis.userIntent.keywords, skill.spec.tags || []);
  if (keywordScore > 0.3) {
...
function compareTaskType(taskType: string, skill: InstalledSkill): number {
  ...
  const specLower =
    `${skill.spec.name} ${skill.spec.description} ${skill.spec.system_prompt}`.toLowerCase();
...
function generateImprovements(
  ...
  const newKeywords = analysis.userIntent.keywords.filter((k) => !existingKeywords.includes(k));

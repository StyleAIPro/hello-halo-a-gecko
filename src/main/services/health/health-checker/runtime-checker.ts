...
export function startFallbackPolling(callback: HealthChangeCallback): void {
  ...
  console.log(
    `[Health][Runtime] Fallback polling started (${FALLBACK_POLL_INTERVAL_MS / 1000}s interval)`,
  );
}
...
async function performFallbackCheck(): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      totalErrors >= 3 ||
      heapUsedMB > MEMORY_CRITICAL_THRESHOLD_MB
    ) {
      newStatus = 'unhealthy';
    ...
    if (newStatus !== lastKnownStatus) {
      const message = issues.length > 0 ? issues.join('; ') : 'All checks passed';
...
async function doImmediateCheck(): Promise<ImmediateCheckResult> {
  ...
  try {
    ...
    const registeredCloudflaredPids = registeredCloudflared
      .map((p) => p.pid)
    ...
    // ========================================
    const routerStatus: ServiceCheckStatus = {
      port: null,
      responsive: false,
    };

    const httpServerStatus: ServiceCheckStatus = {
      port: null,
      responsive: false,
    };
    ...
    const hasServiceIssues =
      (routerInfo && !routerStatus.responsive) ||
      (serverInfo.running && !httpServerStatus.responsive);
    const healthy =
      issues.length === 0 || (registryRemoved > 0 && issues.length === 1 && !hasServiceIssues);
...
export async function runPpidScanAndCleanup(): Promise<{
  ...
  try {
    ...
    for (const entry of registeredClaude) {
      if (entry.pid && !claudeProcesses.includes(entry.pid)) {
        console.log(
          `[Health][Runtime] Cleaning dead claude process: ${entry.id} (PID: ${entry.pid})`,
        );
        unregisterProcess(entry.id, 'v2-session');
        removed++;
      }
    ...
    for (const entry of registeredCloudflared) {
      if (entry.pid && !cloudflaredProcesses.includes(entry.pid)) {
        console.log(
          `[Health][Runtime] Cleaning dead cloudflared process: ${entry.id} (PID: ${entry.pid})`,
        );
        unregisterProcess(entry.id, 'tunnel');
        removed++;
      }

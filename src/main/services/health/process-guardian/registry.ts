...
export function markInstanceStart(): string {
  ...
  console.log(
    `[Health][Registry] Instance started: ${currentInstanceId.slice(0, 8)}... (${duration}ms)`,
  );
...
export function registerProcess(entry: Omit<ProcessEntry, 'lastHeartbeat'>): void {
  ...
  console.log(
    `[Health][Registry] Registered ${entry.type}: ${entry.id} (PID: ${entry.pid ?? 'unknown'})`,
  );
}
...
export function unregisterProcess(id: string, type: ProcessType): void {
  ...
  if (index >= 0) {
    ...
    saveRegistry(registry);
    console.log(
      `[Health][Registry] Unregistered ${type}: ${id} (PID: ${removed.pid ?? 'unknown'})`,
    );
  }
}
...
export function clearOrphanEntries(): void {
  ...
  if (registry.processes.length < originalCount) {
    saveRegistry(registry);
    console.log(
      `[Health][Registry] Cleared ${originalCount - registry.processes.length} orphan entries`,
    );
  }
}

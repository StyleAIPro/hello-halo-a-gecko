  ...
  server.tool(
    ...
    'Get recent output from the shared terminal (user commands and output)',
    {
      lines: z
        ...
        .optional()
        .default(50)
        .describe('Number of lines to retrieve (default: 50)'),
    ...
    async ({ lines, conversationId }) => {
      try {
        ...
        const formatted = outputLines
          .map((line) => {
            const prefix =
              line.source === 'user-command'
                ? '$ '
                : line.source === 'agent-command'
                  ? '[AGENT] $ '
                  : line.source === 'agent-output'
                    ? '  [AGENT] '
                    : '  ';
  ...
  server.tool(
    ...
    'Clear the terminal output buffer (does not affect the actual terminal session)',
    {
      conversationId: z
        .string()
        .optional()
        .describe('Conversation ID (defaults to most recent session)'),
    },

...
export function registerConfigHandlers(): void {
  ...
  ipcMain.handle('config:get', async () => {
    ...
    try {
      ...
      console.log(
        '[Settings] config:get - Loaded, aiSources v2, currentId:',
        decryptedConfig.aiSources?.currentId || 'none',
      );
  ...
  ipcMain.handle('config:refresh-ai-sources', async () => {
    ...
    try {
      ...
      console.log(
        '[Settings] config:refresh-ai-sources - Refreshed, current:',
        (config as any).aiSources?.current || 'custom',
      );
      return { success: true, data: config };
    } catch (error: unknown) {
      const err = error as Error;
  ...
  ipcMain.handle('ai-sources:switch-source', async (_event, sourceId: string) => {
    ...
    try {
      ...
      runConfigProbe().catch((err) =>
        console.error('[Settings] ai-sources:switch-source - Probe failed:', err),
      );
      return { success: true, data: result };
    } catch (error: unknown) {
      const err = error as Error;
  ...
  ipcMain.handle('ai-sources:add-source', async (_event, source: AISource) => {
    ...
    try {
      ...
      runConfigProbe().catch((err) =>
        console.error('[Settings] ai-sources:add-source - Probe failed:', err),
      );
      return { success: true, data: result };
    } catch (error: unknown) {
      const err = error as Error;
  ...
  ipcMain.handle(
    ...
    async (_event, sourceId: string, updates: Partial<AISource>) => {
      ...
      try {
        ...
        emitConfigChange(['aiSources.sources']);
        runConfigProbe().catch((err) =>
          console.error('[Settings] ai-sources:update-source - Probe failed:', err),
        );
        return { success: true, data: result };
      } catch (error: unknown) {
        const err = error as Error;

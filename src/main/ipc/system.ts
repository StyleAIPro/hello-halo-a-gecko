  ...
  ipcMain.handle(
    ...
    async (_event, spaceId: string, conversationId: string) => {
      try {
        ...
        console.log(
          `[Terminal] WebSocket URL generated for space=${spaceId}, conv=${conversationId}`,
        );
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      {
        ...
        conversationId,
        command,
      }: { spaceId: string; conversationId: string; command: string },
    ) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      {
        ...
        conversationId,
        lines,
      }: { spaceId: string; conversationId: string; lines?: number },
    ) => {
      try {
        const { sharedTerminalService } =
          await import('../services/terminal/shared-terminal-service');

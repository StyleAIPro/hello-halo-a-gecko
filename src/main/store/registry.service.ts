...
export function initRegistryService(overrides?: Partial<RegistryServiceConfig>): void {
  ...
  console.log(
    `[RegistryService] Initialized in ${duration.toFixed(1)}ms (${config.registries.length} registries)`,
  );
}
...
export async function refreshIndex(registryId?: string): Promise<void> {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === 'rejected') {
      console.error(
        `[RegistryService] Failed to refresh index for "${registry.name}":`,
        result.reason,
      );
    }
  }
 ...
 */
function resolveEntrySearchText(
  entry: RegistryEntry,
  locale?: string,
...
export async function installFromStore(
  ...
  if (runtime) {
    try {
      ...
      console.warn(
        `[RegistryService] installFromStore: runtime activate failed (non-fatal): ${errMsg}`,
      );
    }
  }

  console.log(
    `[RegistryService] Installed "${entry.name}" (${slug}) as ${appId} in space ${spaceId}`,
  );
  return appId;
}
...
export function addRegistry(registry: Omit<RegistrySource, 'id'>): RegistrySource {
  ...
  const duplicate = config.registries.find((r) => normalizeRegistryUrl(r.url) === normalizedUrl);
  if (duplicate) {
...
export function loadConfig(): RegistryServiceConfig {
  try {
    ...
    const storeConfig = (appConfig as Record<string, unknown>)[CONFIG_KEY] as
    ...
    return {
      ...
      cacheTtlMs: normalizeCacheTtl(
        typeof storeConfig.cacheTtlMs === 'number' ? storeConfig.cacheTtlMs : DEFAULT_CACHE_TTL_MS,
      ),
      autoCheckUpdates:
        typeof storeConfig.autoCheckUpdates === 'boolean' ? storeConfig.autoCheckUpdates : true,
    };
  } catch (error) {
...
function ensureDefaultRegistry(): boolean {
  ...
  if (official) {
    ...
    if (
      wasEnabled !== official.enabled ||
      wasName !== official.name ||
      wasUrl !== official.url ||
      wasDefault !== official.isDefault
    ) {
      changed = true;
    }
...
async function fetchAndCacheIndex(registry: RegistrySource): Promise<RegistryIndex> {
  ...
  try {
    ...
    console.log(
      `[RegistryService] Fetched index from "${registry.name}": ${index.apps.length} apps`,
    );
    return index;
  } finally {

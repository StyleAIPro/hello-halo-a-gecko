...
async function waitForAppManager(maxMs = 5000, intervalMs = 200) {
  ...
  console.error(
    `[AicoBotAppsMcp] AppManager still null after ${maxMs}ms — initPlatformAndApps may have failed`,
  );
  return null;
}
  ...
  const pause_automation_app = tool(
    ...
    async (args) => {
      try {
        ...
        return textResult(
          `Successfully paused app ${args.app_id}. It will not run again until resumed.`,
        );
      } catch (e) {
  ...
  const resume_automation_app = tool(
    ...
    async (args) => {
      try {
        ...
        return textResult(
          `Successfully resumed app ${args.app_id}. It is now active and will run on schedule.`,
        );
      } catch (e) {
  ...
  const update_automation_app = tool(
    ...
    async (args) => {
      try {
        ...
        return textResult(
          `App ${args.app_id} updated successfully. Changed: ${changedFields.join(', ')}.`,
        );
      } catch (e) {
  ...
  const trigger_automation_app = tool(
    ...
    async (args) => {
      try {
        ...
        return textResult(
          `App ${args.app_id} triggered successfully. Outcome: ${result.outcome}.${runIdPart}`,
        );
      } catch (e) {

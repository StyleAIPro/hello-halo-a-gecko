...
export async function sendAppChatMessage(
  ...
  console.log(
    `[AppChat][${appId}] MCP servers: [${Object.keys(mcpServers).join(', ')}], aiBrowser=${usesAIBrowser}`,
  );
  ...
  try {
    ...
    const v2Session = await getOrCreateV2Session(spaceId, conversationId, {
      sdkOptions,
      config: { aiBrowserEnabled: usesAIBrowser },
      workDir,
    });
    ...
    const sessionWriter = spacePath ? openSessionWriter(spacePath, appId, CHAT_RUN_ID) : undefined;

...
export function createAppRuntimeService(deps: AppRuntimeDeps): AppRuntimeService {
  ...
  function buildScheduleTriggerContext(job: SchedulerJob, app: InstalledApp): TriggerContext {
    ...
    return {
      type: 'schedule',
      description:
        `Scheduled run for "${app.spec.name}" (${scheduleDesc}). ` +
  ...
  function buildEventTriggerContext(
    ...
    return {
      type: 'event',
      description:
        `Triggered by event "${eventType}" for "${app.spec.name}". ` +
        `Time: ${new Date().toISOString()}`,
      eventPayload,
    };
  }
  ...
  function buildManualTriggerContext(app: InstalledApp): TriggerContext {
    return {
      type: 'manual',
      description:
        `Manually triggered run for "${app.spec.name}". ` + `Time: ${new Date().toISOString()}`,
    };
  }
  ...
  function buildEscalationTriggerContext(
    ...
    return {
      ...
      description:
        `Follow-up run for "${app.spec.name}" after user responded to escalation. ` +
  ...
  function broadcastAppStatus(appId: string): void {
    try {
      ...
      broadcastToAll('app:status_changed', {
        appId,
  ...
  async function executeWithConcurrency(
    ...
    try {
      ...
      if (result.outcome !== 'error') {
        try {
          ...
          if (existingEntries.length === 0) {
            ...
            console.log(
              `[Runtime][${runTag}] Fallback activity entry created (AI did not call report_to_user)`,
            );

            // Broadcast to clients
            broadcastToAll('app:activity_entry:new', {
              appId: app.id,
              entry: fallbackEntry as unknown as Record<string, unknown>,
            });
            sendToRenderer('app:activity_entry:new', { appId: app.id, entry: fallbackEntry });
          }
        } catch (fallbackErr) {
          console.error(
            `[Runtime][${runTag}] Failed to create fallback activity entry:`,
            fallbackErr,
          );
        }
      }
      ...
      console.log(
        `[Runtime][Notify] output.notify check: config=${JSON.stringify(notifyConfig)}, outcome=${outcome}`,
      );
      if (shouldNotify && outcome !== 'error') {
        try {
          ...
          console.log(
            `[Runtime][Notify] Calling notifyAppEvent: title="${app.spec.name}", bodyLen=${body.length}`,
          );
        ...
        console.log(
          `[Runtime][Notify] Skipped — condition not met (notify=${JSON.stringify(notifyConfig)}, outcome=${outcome})`,
        );
      }
  ...
  function checkEscalationTimeouts(): void {
    try {
      ...
      for (const entry of pendingEscalations) {
        ...
        broadcastToAll('app:activity_entry:new', {
          appId: app.id,
          entry: errorEntry as unknown as Record<string, unknown>,
        });
        ...
        try {
          ...
          console.error(
            `[Runtime] Failed to update status after escalation timeout: app=${app.id}:`,
            statusErr,
          );
        }
  ...
  const service: AppRuntimeService = {
    ...
    async triggerManually(appId: string): Promise<AppRunResult> {
      ...
      const appIsRunning = Array.from(runningAbortControllers.keys()).some((k) =>
        k.startsWith(`${appId}:`),
      );
    ...
    getAppState(appId: string): AutomationAppState {
      ...
      const isRunning = Array.from(runningAbortControllers.keys()).some((k) =>
        k.startsWith(appPrefix),
      );
    ...
    async respondToEscalation(
      ...
      if (app) {
        ...
        executeWithConcurrency(app, trigger).catch((err) => {
          console.error(`[Runtime] Escalation follow-up run failed: app=${appId}:`, err);
        });
      }
    },
    ...
    async activateAll(): Promise<void> {
      ...
      if (!escalationCheckInterval) {
        ...
        console.log(
          `[Runtime] Escalation timeout checker started (interval=${ESCALATION_CHECK_INTERVAL_MS / 60000}m)`,
        );
      }

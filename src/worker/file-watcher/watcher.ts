...
export function setOnEventsCallback(
...
async function processParcelEvents(sw: SpaceWatcher, events: ParcelEvent[]): Promise<void> {
  // Filter
  ...
  await Promise.all(
    filtered.map(async (event) => {
      ...
      const changeType = event.type === 'create' ? (isDir ? 'addDir' : 'add') : 'change';

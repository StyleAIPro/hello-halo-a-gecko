...
| 日期 | 内容 | 指令人 | 触发来源 |
|------|------|--------|---------|
| 2026-05-13 | 修复远程服务器管理三个 UX bug：1) 删除服务器后 IPC handler 未 await removeServer 导致列表不刷新；2) SSH 认证错误无明确提示，新增 authError 标记和 error 级别通知；3) 密码修改后旧 SSHManager 缓存导致操作锁死锁，updateServer 检测密码变更时销毁旧 manager，SSHManager error/close 事件重置 `_forceDisconnected` 和 `_operationLock` | @mi-saka | bugfix-remote-server-ux-v1 |

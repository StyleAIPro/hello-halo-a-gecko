...
| 日期 | 内容 | 指令人 | 触发来源 |
| 2026-05-07 | 修复断网后 WebSocket 无法重连：(1) reconnectTimer 触发后未清空导致重连只执行一次而非 5 次；(2) 连接池 acquireConnection 直接 destroy 正在重连的客户端；(3) 连接池监听 close 而非 disconnected/reconnectFailed 事件；(4) code:undefined 归一化为 1006 — `src/main/services/remote/ws/remote-ws-client.ts`, `ws-connection-pool.ts` — PRD: `prd/bugfix/remote/bugfix-ws-reconnect-v1` | @moonseeker1 | bugfix-ws-reconnect-v1 |
|------|------|--------|---------|

...
### 中途发消息流程（isGenerating 时的排队机制）

> **[2026-04-17 变更]** 回退方案：不再触发 interrupt，改为排队等待自然完成。

1. 用户在 Agent 生成过程中发送消息
2. 前端 `sendMessage()` 检测到 `isGenerating === true`
3. 消息入队 `pendingMessages`，**不触发** `stopGeneration()` interrupt
4. 当前任务自然运行至完成
5. `handleAgentComplete` 触发时检测 `pendingMessages` 队列
6. 队列非空则自动发送下一条消息作为新请求（使用现有 pendingMessages 处理逻辑）
7. 远端同理：消息入队，不触发 interrupt，stream 完成后处理排队消息
